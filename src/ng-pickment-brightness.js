...
angular.module('pickment-brightness', ['pickment'])
  .run(['pickmentService', function(pickmentService) {
      ...
      this.elements = {
        ...
        primary: {
          ...
          render: function(canvas, col) {
            ...
            for (var h = 0, hstep = 60 ; h <= 360 ; h += hstep) {
              gradientH.addColorStop(h / 360, tinycolor({h: h, s: 1, v: col.toHsv().v, a: 1}).toRgbString());
            }
            ctx.fillStyle = gradientH;
            ctx.fillRect(0, 0, width, height);
            col = tinycolor({h: 0, s: 0, v: col.toHsv().v});
            gradientW.addColorStop(0, col.setAlpha(0).toRgbString());
            gradientW.addColorStop(1, col.setAlpha(1).toRgbString());
            ctx.fillStyle = gradientH;
            ctx.fillRect(0, 0, width, height);
          ...
          getColor: function(x, y, c) {
            return tinycolor({h: x * 360, s: 1 - y, v: c.toHsv().v});
          },
        ...
        secondary: {
          ...
          render: function(canvas, col) {
            ...
            col = col.toHsv();
            gradient.addColorStop(0, tinycolor({h: col.h, s: col.s, v: 1, a: 1}).toRgbString());
            gradient.addColorStop(1, tinycolor({h: col.h, s: col.s, v: 0, a: 1}).toRgbString());
            ctx.fillStyle = gradient;
            ctx.fillRect(0, 0, width, height);
          },
          getColor: function(x, y) {
            return tinycolor({h: positions.primary.x * 360, s: 1 - positions.primary.y, v: 1 - y});
      },
          getPosition: function(c) {
            return {
              x: 0,
    ...
    pickmentService.getResource('alphaPatternImage')
      .then(function(alphaPatternImage) {
        pickmentService.registerMode('brightness', new mode(alphaPatternImage));
      });
  }]);